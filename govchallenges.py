...
#CREATE CSV
filename = "test11.csv"
f = io.open(filename, "w", errors = 'replace')
...
for year_link in years:
    ...
    for chal in chals:
        ...
        funds_start = challenge_details.find('$')
        if funds_start != -1:
            funds_end = challenge_details[funds_start:].find("\n")
            chal_funds = challenge_details[funds_start+1:]
            chal_funds = chal_funds[0:funds_end-1]
        else:
            chal_funds = ""
...
#       (as done to get the submission date, cash prize, and agencies) so that there is less repeating/similar code.
        ...
        #federal
        fed_start = challenge_details.find("Partner Agencies | Federal: ")
        if fed_start != -1:
            fed_agencies = challenge_details[fed_start+28:] #len = 28
            fed_end = fed_agencies.find("\n")
            fed_agencies = fed_agencies[0:fed_end]
        else:
            fed_agencies = ""
        #non federal
        nfed_start = challenge_details.find("Non-federal: ") #len = 13
        if nfed_start != -1:
            nfed_agencies = challenge_details[nfed_start+13:]
            nfed_end = nfed_agencies.find("\n")
            nfed_agencies = nfed_agencies[0:nfed_end]
        else:
            nfed_agencies = ""
        #concatenate
        if fed_agencies == "" or nfed_agencies == "":
        ...
        #FIX: 'NoneType' object has no attribute 'findAll'
        chal_link2 = chal_link
        link_list = chalpg_soup.main.find("div",{"class":"desktop:grid-col-9 usa-prose usa-section challenge-main"})
        if link_list is not None:
            link_list = link_list.findAll("a")
            if link_list != []:
        ...
        #FIX: test the ascii values first to make sure writeable
